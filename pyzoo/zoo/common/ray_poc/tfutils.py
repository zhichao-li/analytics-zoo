...
class TensorFlowVariables(object):
    ...
    def __init__(self, output, sess=None, input_variables=None):
        ...
        # global_trainable_variable_names = [ v.op.node_def.name for v in tf.global_variables()]
        global_trainable_variable_names = [ v.op.node_def.name for v in tf.trainable_variables()]
        ...
        while len(queue) != 0:
            ...
            if tf_obj.node_def.name in global_trainable_variable_names:
                variable_names.append(tf_obj.node_def.name)
