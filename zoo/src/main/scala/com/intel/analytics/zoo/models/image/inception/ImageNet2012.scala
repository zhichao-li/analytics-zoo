...
object ImageNet2012 {
  ...
  def apply(
    ...
    coresPerNode: Int,
    classNumber: Int,
    cacheWithAEP: Boolean = false
  )
  : DataSet[MiniBatch[Float]] = {
    val rawData = readFromSeqFiles(path, sc, classNumber)
    val cachedDataSet: DistributedDataSet[ByteRecord] = if (cacheWithAEP) {
      println("~~~~~~~ Caching with AEP support ~~~~~~~")
      ImageSet.cacheWithAEP(rawData)
    } else {
      DataSet.rdd(rawData)
    }
